...
impl Memory {
    ...
    pub fn read(&self, address: u16, cpu: &Cpu, ppu: &mut Ppu) -> Result<u8, MemoryError> {
        ...
        if value.is_ok() {
            ...
            println!("Read memory byte at address 0x{:04X}: 0x{:02X}", address, tmp);
            return Ok(tmp);
        } else {
            println!("Read memory byte at address 0x{:04X}: FAILED", address);
        }
        return value;
    }
...
impl Cartridge {
    ...
    fn new(rom_bytes: &[u8]) -> Result<Cartridge, RomError> {
        ...
        if header.charactor_memory_size != 0 {
            cartridge_chr_rom.append(&mut rom_bytes[(prg_rom_end_index)..(prg_rom_end_index - 256)].to_vec());
        } else {
            ...
            1 => {
                match self.prg_bank_mode {
                    ProgramBankMode::Fullswitch => {
                        ...
                        match address {
                            0x6000..0x8000 => Ok(self.pgr_ram[(address - 0x6000) as usize]), // PGR RAM
                            0x8000..0xff00 => Ok(self.prg_data
                    ...
                    ProgramBankMode::Fixlast => {
                        match address {
                            ...
                            0x8000..0xc000 => Ok(self.prg_data[(address - 0x8000
                                ...
                                as usize]), // make 0x8000 - 0xc000 switchable
                            0xc000..0xff00 => {
                                let target: u32 = address as u32 - 0xc000 + ((self.header.program_rom_size - 1) as u32) * 16384;
